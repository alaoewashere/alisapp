...
# -*- coding: utf-8 -*-
"""
Sello Design System Generator - Aggregates search results and applies reasoning
to generate comprehensive design system recommendations for the Sello brand.
...
SELLO_FONT_DIR = "assets/fonts"

SELLO_TYPOGRAPHY = {
    "heading": "Thmanyah Serif Display",
    "body": "Thmanyah Sans",
    "longform": "Thmanyah Serif Text",
    "mood": "Editorial, premium, Arabic-first brand voice",
    "best_for": "Sello brand surfaces",
}

THMANYAH_FONT_FAMILIES = [
    ("ThmanyahSans", "thmanyahsans", "thmanyahsans"),
    ("ThmanyahSerifDisplay", "thmanyahserifdisplay", "thmanyahserifdisplay"),
    ("ThmanyahSerifText", "thmanyahseriftext", "thmanyahseriftext"),
]

THMANYAH_FONT_WEIGHTS = [
    ("Light", 300),
    ("Regular", 400),
    ("Medium", 500),
    ("Bold", 700),
...
def _thmanyah_font_face() -> str:
    """Build @font-face rules for bundled Thmanyah OTF files."""
    blocks = []
    for css_family, folder, prefix in THMANYAH_FONT_FAMILIES:
        for weight_name, weight_num in THMANYAH_FONT_WEIGHTS:
            filename = f"{prefix}-{weight_name}.otf"
            path = f"{SELLO_FONT_DIR}/{folder}/{filename}"
            blocks.append(
                "@font-face {\n"
                f"  font-family: '{css_family}';\n"
                f"  src: url('{path}') format('opentype');\n"
...
def _build_sello_typography() -> dict:
    """Return Sello brand typography (local Thmanyah fonts, no Google Fonts)."""
    return {
        "heading": SELLO_TYPOGRAPHY["heading"],
        "body": SELLO_TYPOGRAPHY["body"],
        "longform": SELLO_TYPOGRAPHY["longform"],
        "heading_family": "ThmanyahSerifDisplay",
        "body_family": "ThmanyahSans",
        "longform_family": "ThmanyahSerifText",
        "mood": SELLO_TYPOGRAPHY["mood"],
        "best_for": SELLO_TYPOGRAPHY["best_for"],
        "font_dir": SELLO_FONT_DIR,
        "font_face_css": _thmanyah_font_face(),
    }
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        return {
            ...
            "typography": _build_sello_typography(),
...
def format_ascii_box(design_system: dict) -> str:
    """Format design system as ASCII box with text icons."""
    ...
    if typography.get("longform"):
        lines.append(
            f"|     Long-form: {typography.get('longform', '')}".ljust(BOX_WIDTH) + "|"
        )
    ...
    if typography.get("font_dir"):
        lines.append(
            f"|     Local fonts: {typography.get('font_dir', '')}/thmanyah*".ljust(BOX_WIDTH) + "|"
        )
    lines.append("|" + " " * BOX_WIDTH + "|")
...
def format_markdown(design_system: dict) -> str:
    ...
    lines.append(f"- **Heading:** {typography.get('heading', '')} (`{typography.get('heading_family', '')}`)")
    lines.append(f"- **Body:** {typography.get('body', '')} (`{typography.get('body_family', '')}`)")
    lines.append(
        f"- **Long-form:** {typography.get('longform', '')} (`{typography.get('longform_family', '')}`)"
    )
    if typography.get("mood"):
    ...
    if typography.get("font_dir"):
        lines.append(f"- **Local fonts:** `{typography.get('font_dir', '')}/`")
    if typography.get("font_face_css"):
        lines.append("- **@font-face (bundled OTF):**")
        lines.append("```css")
        lines.append(typography.get("font_face_css", ""))
        lines.append("```")
    lines.append("")
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("### Typography")
    lines.append("")
    lines.append(f"- **Heading Font:** {typography.get('heading', 'Thmanyah Serif Display')}")
    lines.append(f"- **Body Font:** {typography.get('body', 'Thmanyah Sans')}")
    lines.append(f"- **Long-form Font:** {typography.get('longform', 'Thmanyah Serif Text')}")
    if typography.get("mood"):
        lines.append(f"- **Mood:** {typography.get('mood', '')}")
    if typography.get("font_dir"):
        lines.append(f"- **Local fonts:** `{typography.get('font_dir', '')}/`")
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- [X] {anti}")
    lines.append("")
    lines.append("### Additional Forbidden Patterns")
    lines.append("")
    lines.append("- [X] **Emojis as icons** — Use SVG icons (Heroicons, Lucide, Simple Icons)")
    lines.append("- [X] **Missing cursor:pointer** — All clickable elements must have cursor:pointer")
    lines.append("- [X] **Layout-shifting hovers** — Avoid scale transforms that shift layout")
    lines.append("- [X] **Low contrast text** — Maintain 4.5:1 minimum contrast ratio")
    lines.append("- [X] **Instant state changes** — Always use transitions (150-300ms)")
    lines.append("- [X] **Invisible focus states** — Focus states must be visible for a11y")
    lines.append("")
