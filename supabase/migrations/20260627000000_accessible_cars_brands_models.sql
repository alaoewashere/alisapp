-- سيارات ذوي الاحتياجات الخاصة (veh_accessible) — copy brand/model tree from سيارات
-- Safe to re-run: replaces veh_accessible subtree; slugs veh_auto_br_* → veh_accessible_br_*

CREATE OR REPLACE FUNCTION public._copy_auto_tree_to_accessible() RETURNS VOID AS $$
DECLARE
  v_source_id INT;
  v_target_id INT;
  brand_rec RECORD;
...
BEGIN
  ...
  SELECT id INTO v_target_id FROM public.categories WHERE slug = 'veh_accessible';

  IF v_source_id IS NULL THEN
    RAISE EXCEPTION 'Source category not found: veh_automobile';
  END IF;
  IF v_target_id IS NULL THEN
    RAISE EXCEPTION 'Target category not found: veh_accessible';
  END IF;
  ...
  LOOP
    v_new_slug := replace(brand_rec.slug, 'veh_auto_br_', 'veh_accessible_br_');
    ...
    FOR model_rec IN
      ...
      ORDER BY display_order, id
    LOOP
      v_new_slug := replace(model_rec.slug, 'veh_auto_br_', 'veh_accessible_br_');
...
SELECT public._copy_auto_tree_to_accessible();

DROP FUNCTION public._copy_auto_tree_to_accessible();
